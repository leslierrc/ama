...
async function main() {
  try {
    ...
    // 1. Delete combo items
    console.log("Deleting combo items...");
    await fetch(`${baseUrl}/rest/v1/combo_items?id=not.is.null`, {
      method: 'DELETE',
      headers
    });
    ...
    const delRes = await fetch(`${baseUrl}/rest/v1/products?id=not.is.null`, {
      method: 'DELETE',
      headers
    });
    if (!delRes.ok) {
    ...
    const insertRes = await fetch(`${baseUrl}/rest/v1/products`, {
      method: 'POST',
      headers: {
        ...headers,
    ...
    if (!insertRes.ok) {
      ...
      throw new Error(`Failed to insert products: ${txt}`);
    }

    const insertedData = await insertRes.json();
    console.log(`Successfully added ${insertedData.length} products to database!`);
  } catch (error) {
